...
async def test_wallet_flow(client, db_session):
    ...
    result = await db_session.execute(select(Wallet).where(Wallet.user_id == uuid.UUID(user_id)))
    ...
    response = await client.get(f"/api/v1/wallets/{uuid.UUID(wallet_id)}", headers=headers)
    assert response.status_code == 200
